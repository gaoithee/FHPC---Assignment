...
- if specified by the user in the command line through `-i` command, the world is initialized by `-np x` processes. This consists in subdividing the randomic generation of the rows of the matrix between all the processes. When all of them have finished, each set of lines is sent to process 0 who will be responsible of writing on a file. This is what does the file `w_init.c`.
...
In the `main` function, the function `choose_initialization` is called. This defines a `world` of size `size*size` and of type `unsigned int`. We preferred to use `char`s instead of integers or double or whatever because they belong to the smallest type that is able to contain both `0`s and `255`s, that are the outputs of `read_write_pgm.c` functions. 
...
The `updated_world` function has the aim of selecting a cell, look at its neighbors and establish whether the unit must live or die. 
...
Due to performance reasons, a double pointer is used to determine which world is the reading one and which one is the updating one. 
...
library(ggplot2)
library(plyr)
dataset <- read.table("~/Scrivania/robe/dataset.dat", quote="\"", comment.char="")
colnames(dataset) <- c("SOCKET", "POLICY", "PLACE", "DIM", "THREADS", "TIME", "SPEEDUP")

dataset2 <- read.table("~/Scrivania/robe/graph 1 - OMP - 1s.dat", quote="\"", comment.char="")
...
```{r, echo=F}
two_skt <- read.table("~/Scrivania/robe/graph 2 - OMP - 2s.dat", quote="\"", comment.char="")
...
four_skt <- read.table("~/Scrivania/robe/graph 2 - OMP - 4s.dat", quote="\"", comment.char="")
...
six_skt <- read.table("~/Scrivania/robe/graph 2 - OMP - 6s.dat", quote="\"", comment.char="")
...
dataset2_omp <- rbind(two_skt, four_skt, six_skt)
...
library(ggplot2)
library(plyr)
dataset <- read.table("~/Scrivania/robe/graph 3 - MPI - scatter.dat", quote="\"", comment.char="")
...
```{r, echo=F}
noscatterdata <- read.table("~/Scrivania/robe/graph 4 - MPI - noscatter.dat", quote="\"", comment.char="")
...
scatterdata <- read.table("~/Scrivania/robe/graph 3 - MPI - scatter.dat", quote="\"", comment.char="")
...
```{r}
noscatter25k <- read.table("~/Scrivania/robe/graph 4 - MPI - noscatter.dat", quote="\"", comment.char="")
...
noscatter20k <- read.table("~/Scrivania/robe/graph 5 - MPI - noscatter.dat", quote="\"", comment.char="")
...
On the opposite, the greater downfall that can be observed for 25.000x25.000 matrices with almost 140 MPI tasks seems only to be an unfortunate measure. 
...
We consider the no-scatter version of `grow_static.c`.
If we plot time needed with different MPI tasks and matrices sizes, in order to keep the workload constant, we obtain:

```{r}
weak <- read.table("~/Scrivania/robe/graph 6 - weak.dat", quote="\"", comment.char="")
...
```{r, echo=F, message=F}
data <- read.table("~/Scrivania/robe/graphs - DATA - HPC.dat", quote="\"", comment.char="", header = T)
...
data_NONE <- read.table("~/Scrivania/robe/graphs - DATA - HPC - NONE.dat", quote="\"", comment.char="")
...
```{r}
data <- read.table("~/Scrivania/robe/graphs - DATA - HPC - 2.dat", quote="\"", comment.char="")
