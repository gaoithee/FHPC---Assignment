...
In the `main` function, the function `choose_initialization` is called. This defines a `world` of size `size*size` and of type `unsigned char`. We preferred to use `char`s instead of integers or double or whatever because they belong to the smallest type that is able to contain both `0` and `255`, that are the outputs of `read_write_pgm.c` functions. 
...
***non so cosa commentare EDIT: riproviamo i benchmark rivedendo il batch, eventualmente togliamo***
